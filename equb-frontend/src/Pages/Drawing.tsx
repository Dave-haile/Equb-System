...
export default function Drawing() {
    ...
    const { data: draws = [], isLoading: drawsLoading } = useQuery({
        ...
        queryFn: () => base44.entities.DrawResult.filter({ group_id: activeGroupId }, "-round_number"),
        enabled: !!activeGroupId,
    });

    const saveDraw = useMutation({
        mutationFn: (data) => base44.entities.DrawResult.create(data),
    ...
    const eligibleMembers = members.filter((m) => !m.has_won && m.status === "active");
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5">Select the winner for the current round</p>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    ...
                    <Card className="border bg-card/80">
                        <CardContent className="p-4 text-center">
                            ...
                            <p className="text-2xl font-bold mt-1 text-amber-500">{totalPool}g</p>
                            <p className="text-[10px] text-muted-foreground">{activeGroup.contribution_karat}K Gold</p>
                        </CardContent>
                    </Card>
                    <Card className="border bg-card/80 col-span-2 sm:col-span-1">
                        <CardContent className="p-4 text-center">
                            <p className="text-xs text-muted-foreground">Eligible Members</p>
                            <p className="text-2xl font-bold mt-1">{eligibleMembers.length}</p>
                        </CardContent>
                    </Card>
                </div>
            )}
            ...
            <Card className="border bg-card/80">
                ...
                <CardContent>
                    {drawsLoading ? (
                        ...
                        <p className="text-center py-8 text-sm text-muted-foreground">No draws yet</p>
                    ) : (
                            ...
                            {draws.map((draw) => (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                {moment(draw.draw_date || draw.created_date).format("MMM D, YYYY")}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">{draw.total_pool_grams}g pool</p>
                                    </div>
                                </div>
                            ))}
                        </div>
